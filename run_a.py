...
# ==========================================
# 2. V60.14 狙击引擎 (抓捕起跳前夜)
# ==========================================
def analyze_stock_v14(df_s, bench_series, t_code):
    try:
        ...
        # 3. 核心抓手：波动挤压与成交量枯竭
        v_r = float(v.iloc[-1] / v.iloc[-21:-1].mean()) # 今日量比
        amp = (float(h.iloc[-1]) - float(l.iloc[-1])) / prev * 100 # 今日振幅
        dist_h = ((curr / h.tail(10).max()) - 1) * 100 # 距离10日高点的距离
        
        # 4. 逻辑判定：谁是明天的黑马？
        level, act, win, guide = "⚪ 观察", "潜伏观察", "45%", "无明显信号"
        
        # 条件 A：静默期狙击 (VCP收缩) -> 专门抓起跳前夜
        # 逻辑：RS极高 + 距离高点不远 + 振幅极小(窄幅) + 成交量极度萎缩
        is_squeeze = amp < 2.5 and v_r < 0.65
        is_strong_trend = curr > ma20 and ma20 > ma50
        
        if rs > 90 and is_strong_trend:
            if is_squeeze and -3 < dist_h <= 0:
                level, act, win, guide = "🚀 进攻", "静默狙击(起跳点)", "88%", "极窄波动+地量,随时爆发"
            elif ma20 * 0.99 <= curr <= ma20 * 1.01:
                level, act, win, guide = "🎯 进攻", "生命线停车", "82%", "20日线精准支撑,地量关注"
            else:
                level, act, win, guide = "🟡 准备", "强势整理", "65%", "趋势向上,等窄幅横盘"

        # 条件 B：经典毒蛇 (深蹲坑)
        elif rs > 75 and ma50 * 0.98 <= curr <= ma50 * 1.02:
            if v_r < 0.6:
                level, act, win, guide = "🐍 潜伏", "毒蛇出洞", "75%", "50日线地量潜伏"

        return [t_code, act, level, f"{dist_h:.1f}%", f"{v_r:.2f}x", f"{amp:.1f}%", 
                "多头排列" if (ma20 > ma50) else "震荡", 
                win, guide, f"{rs:.2f}"]
    except: return None
...
def main():
    ...
    trace_id = f"VIPER-V14-{uuid.uuid4().hex[:4].upper()}"
    
    print(f"🚀 V60.14 抓捕前夜版启动 | ID: {trace_id}")
    ...
    for t_full in tickers:
        ...
        try:
            res = analyze_stock_v14(data[t_full], bench, t_raw)
    ...
    header = [
        ["🚀 V60.14 黎明狙击版", "ID:", trace_id, "策略:", "静默收缩+高位起跳", "更新:", dt_str, "", "", ""], 
